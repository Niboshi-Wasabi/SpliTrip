...
async function AdminUsersDashboard() {
  const usersResult = await listAdminUsers()
    .then((users) => ({ users, fetchError: null as null | unknown }))
    .catch((fetchError) => ({ users: null as null, fetchError }));

  if (usersResult.fetchError || !usersResult.users) {
    const error = usersResult.fetchError;
