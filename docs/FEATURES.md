...
- **マーケ LP:** トップ（`/`、`LandingPage`）に AI 訴求のヒーロー（Framer Motion：ヒーロー登場・アプリ枠モックの浮き下線付き機能カードのアニメ等）、差別化ポイント／主要機能、フリーミアム料金（Free / PRO）、CTA。背景グラデーションはログイン画面と共通（`LANDING_PAGE_BACKGROUND_CLASSNAME`）。**ログイン済みセッションをLP上でも保持**し、ヘッダー導線をログイン→アカウントアイコン＋ダッシュボード導線へ切り替える。ログイン後はアバターメニューから **ダッシュボード / 設定 / ログアウト** へ遷移可能。One Tap 成功時は LP をリロードして同一ページ上で導線を更新。**未ログイン時のみ**、`LandingWhatsNew` が `GET /api/public/announcements` の **先頭1件** を NEW バナーとして表示し、クリックで `WhatsNewModal`（Markdown・**既読は `localStorage` のみ**）を開く。**ログイン済み**はレイアウトの `WhatsNewModalGate` を優先しバナーを出さない。文言は `messages/*.json` の `Landing` / `LandingV2` 名前空間。
...
| **システムステータス** | `system_status` にサービス別 `status`・`updated_at`・`pinned_by_admin`（`true` の行は定期ヘルスチェックが `status` を上書きしない）を保存。公開 **`/{locale}/status`** と **`GET /api/public/system-status`**（`messages.Status`）。SSR 後も **約60秒ごと**にパブリック API をブラウザ再取得。運営は **`GET` / `PUT /api/admin/status`** と管理フォームで編集（監査 `system_status_update`。**`pinned_before` / `pinned_after`** も記録）。**`GET /api/cron/system-status`** は **`Authorization: Bearer <CRON_SECRET>`**＋Vercel の **`CRON_SECRET`** で保護。本番の定期実行は **GitHub Actions**（`.github/workflows/system-status-cron.yml`、概ね **5分毎 UTC**。Secrets: **`SYSTEM_STATUS_CRON_URL`**, **`CRON_SECRET`**）。**Vercel Hobby** ではプラットフォーム Cron が日次までに制限されるため **`vercel.json` に Cron を定義しない**。**Web Push** は現状プレースホルダのため常に正常。**キー未設定**の Stripe/Gemini は異常扱いにしない。全画面メンテ時も Proxy が **`/status`** を通過。フッターから導線。 |
...
| **Web Push の実プローブ** | `POST /api/notifications/web-push` が本番送信に対応したあと、VAPID 設定や購読ストレージの死活をプローブ対象に含める（現状はプレースホルダのため定期プローブは常に `operational`）。 |
...
| **スケジューラ / ポーリング間隔** | GitHub Actions の cron 間隔や公開ページの `refreshInterval`（現状 60 秒）の環境変数化。**Vercel Pro** であれば `vercel.json` の Cron に戻す選択肢あり。 |
| **監視連携** | UptimeRobot 等からの Webhook で `system_status` を更新、またはプローブ結果を Sentry / ログ基盤へ送る。 |
| **管理 UI** | ピン留めの一括解除、プローブ最終実行時刻の表示、手動「今すぐプローブ」ボタン（`GET /api/cron/system-status` と同ロジックの管理者専用 API）。 |
...
| **出費詳細** | モーダル：**内容（description）・日付・分類（カテゴリ）の編集上書き保存**、**領収書**、**削除**、**監査ログ**、**コメント投稿・一覧**（API 経由）。 |
