...
| **グループ詳細** | `/dashboard/groups/[groupId]`：メンバー・アバター、通貨、招待、出費・グラフ・精算・エクスポートを一画面に集約。**オーナーはグループ名と期間（開始日/終了日）をインライン編集可能**（`PATCH /api/groups/[groupId]`）。ヘッダーに **設定画面へのショートカット** を常設。 |
